...
export async function wipeUser(prisma: PrismaClient, userId: string): Promise<void> {
  await prisma.$transaction([
    ...
    prisma.trackEdge.deleteMany({ where: { userId } }),
    prisma.track.deleteMany({ where: { userId } }),
    prisma.referenceDoc.deleteMany({ where: { userId } }),
    prisma.workspaceFile.deleteMany({ where: { userId } }),
    prisma.llmCall.deleteMany({ where: { userId } }),
